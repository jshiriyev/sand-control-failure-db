...
def test_submit_happy_path_creates_rows_in_all_three_tables(client, seeded_org, db_session):
    ...
    assert len(well.production_intervals) == 1
    assert len(well.production_intervals[0].completion_intervals) == 1
...
def test_submit_out_of_range_number_returns_422(client, seeded_org):
    _, token = seeded_org
    payload = _minimal_valid_payload()
    payload["well"]["Production Zone Details"] = {"Production Zones": {"Number of production intervals": 999}}
    resp = client.post("/records", json=payload, headers=_auth(token))
    assert resp.status_code == 422
...
def test_submit_multi_number_splits_into_columns(client, seeded_org, db_session):
    ...
    payload["production_intervals"][0]["completion_intervals"] = [
        {"Drilling": {"Drilling Details": {"Mud PSD": [10, 50, 90]}}}
    ]
    resp = client.post("/records", json=payload, headers=_auth(token))
    assert resp.status_code == 201, resp.text
    ...
    ci = well.production_intervals[0].completion_intervals[0]
...
def test_submit_requires_at_least_one_production_interval(client, seeded_org):
    _, token = seeded_org
    payload = _minimal_valid_payload()
    payload["production_intervals"] = []
    resp = client.post("/records", json=payload, headers=_auth(token))
    assert resp.status_code == 422
